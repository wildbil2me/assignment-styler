...
export default function Home() {
  ...
  const insertIntoBlackbaud = async () => {
    ...
    try{
      ...
      const results=await chromeApi.scripting.executeScript({target:{tabId:tab.id,allFrames:true},args:[html],func:(markup:string)=>{
        ...
        const candidates=Array.from(document.querySelectorAll<HTMLElement>('textarea,[contenteditable="true"],body.mce-content-body,.ck-editor__editable'));
        const editor=(active&&candidates.includes(active)&&visible(active)?active:candidates.find(visible));
        if(!editor)return {inserted:false,reason:"No visible text editor found"};
        editor.focus();
        if(editor instanceof HTMLTextAreaElement||editor instanceof HTMLInputElement){
          const setter=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(editor),"value")?.set;setter?.call(editor,markup);
        }else{editor.innerHTML=markup}
        for(const name of ["input","change","blur"]){editor.dispatchEvent(new Event(name,{bubbles:true}))}
        return {inserted:true,tag:editor.tagName};
      }});
      if(!results.some((r:{result?:{inserted?:boolean}})=>r.result?.inserted))throw new Error("No editor found");
      setExportHistory(v=>[{date:new Date().toLocaleString(),title:`${postTitle} · Inserted`,html},...v].slice(0,10));
  ...
  return <main className={`app-shell ${isExtension?"extension-shell":""}`}>
    ...
    <section className="workspace">
      ...
      <aside className="inspector">
        ...
        <div className="export">{isExtension&&<button className={`insert-button ${insertStatus}`} onClick={insertIntoBlackbaud} disabled={insertStatus==="working"}>{insertStatus==="working"?"Finding Blackbaud editor…":insertStatus==="success"?"✓ Inserted into Blackbaud":insertStatus==="error"?"Editor not found — try source mode":"Insert into Blackbaud"}</button>}<button className="copy" onClick={copy}>{copied?"✓ Copied to clipboard":"Copy for Blackbaud"}</button><details><summary>View generated HTML</summary><textarea readOnly value={html}/></details>{exportHistory.length>0&&<details><summary>Export history ({exportHistory.length})</summary><div className="export-history">{exportHistory.map((x,i)=><button key={`${x.date}-${i}`} onClick={()=>navigator.clipboard.writeText(x.html)}><strong>{x.title}</strong><small>{x.date} · Click to copy</small></button>)}</div></details>}<small>{isExtension?"Insertion never publishes or saves the Blackbaud page. Preview before publishing.":"Paste into Blackbaud’s HTML editor, then preview before publishing."}</small></div>
      </aside>
    </section>
  </main>;
}
