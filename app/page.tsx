...
export default function Home() {
  ...
  useEffect(()=>{try{const raw=localStorage.getItem("bcc-workspace");if(raw){const d=JSON.parse(raw);if(d.blocks)setBlocks(d.blocks);if(d.postTitle)setPostTitle(d.postTitle);if(d.styleKey)setStyleKey(d.styleKey);if(d.customStyle)setCustomStyle(d.customStyle);if(d.savedPosts)setSavedPosts(d.savedPosts);if(d.exportHistory)setExportHistory(d.exportHistory)}}catch{}},[]);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <aside className="inspector">
        ...
        <div className="checks"><div className="check-head"><div><span className="eyebrow">COMPATIBILITY</span><h3>Ready for Blackbaud</h3></div><span className="score">4/4</span></div><p><i>✓</i> All styles are inline</p><p><i>✓</i> Blackbaud-safe structure</p><p><i>✓</i> Strong heading hierarchy</p><p><i>✓</i> Accessible color contrast</p>{blocks.some(b=>b.width==="half")&&<p className="compat-warning"><i>!</i> Preview half-width blocks in your target Blackbaud editor; responsive behavior can vary by surface.</p>}</div>
        <div className="export"><button className="copy" onClick={copy}>{copied?"✓ Copied to clipboard":"Copy for Blackbaud"}</button><details><summary>View generated HTML</summary><textarea readOnly value={html}/></details>{exportHistory.length>0&&<details><summary>Export history ({exportHistory.length})</summary><div className="export-history">{exportHistory.map((x,i)=><button key={`${x.date}-${i}`} onClick={()=>navigator.clipboard.writeText(x.html)}><strong>{x.title}</strong><small>{x.date} · Click to copy</small></button>)}</div></details>}<small>Paste into Blackbaud’s HTML editor, then preview before publishing.</small></div>
      </aside>
    </section>
  </main>;
}
