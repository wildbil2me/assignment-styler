...
type SurfaceKey = "bulletin" | "topic" | "assignment" | "announcement";

const surfaces: Record<SurfaceKey,{name:string;width:number;note:string}> = {
  bulletin:{name:"Bulletin Board",width:720,note:"Balanced layout for longer class pages."},
  topic:{name:"Topic",width:760,note:"Wider topic content with structured sections."},
  assignment:{name:"Assignment",width:680,note:"Compact instructions with prominent actions and deadlines."},
  announcement:{name:"Announcement",width:620,note:"Narrow, concise layout; use full-width blocks when possible."},
};
...
function renderHtml(blocks: Block[], preset: StylePreset, surface: typeof surfaces[SurfaceKey]) {
  const base = `font-family:${preset.body};color:#1F2937;font-size:14px;line-height:1.6;`;
  return `<div style="${base}max-width:${surface.width}px;margin:0 auto;">` + blocks.filter(b=>!b.hidden).map((b) => {
...
export default function Home() {
  ...
  const html = useMemo(() => renderHtml(blocks, preset, surface), [blocks, preset, surface]);
  const active = blocks.find(b => b.id === selected);

  useEffect(()=>{try{const raw=localStorage.getItem("bcc-workspace");if(raw){const d=JSON.parse(raw);if(d.blocks)setBlocks(d.blocks.map(({animation,...b}:Block&{animation?:string})=>b));if(d.postTitle)setPostTitle(d.postTitle);if(d.styleKey)setStyleKey(d.styleKey);if(d.surfaceKey)setSurfaceKey(d.surfaceKey);if(d.customStyle)setCustomStyle(d.customStyle);if(d.savedPosts)setSavedPosts(d.savedPosts.map((p:{id:number;title:string;blocks:Array<Block&{animation?:string}>})=>({...p,blocks:p.blocks.map(({animation,...b})=>b)})));setExportHistory([])}}catch{}},[]);
  ...
  useEffect(()=>{const timer=setTimeout(()=>{localStorage.setItem("bcc-workspace",JSON.stringify({blocks,postTitle,styleKey,surfaceKey,customStyle,savedPosts,exportHistory}))},250);return()=>clearTimeout(timer)},[blocks,postTitle,styleKey,surfaceKey,customStyle,savedPosts,exportHistory]);
  ...
  const importExistingHtml = () => {
    const doc=new DOMParser().parseFromString(importSource,"text/html");
    doc.querySelectorAll("script,style,iframe,object,embed,form").forEach(el=>el.remove());
    let elements=Array.from(doc.body.children) as HTMLElement[];
    if(elements.length===1&&["DIV","MAIN","SECTION","ARTICLE"].includes(elements[0].tagName)&&elements[0].children.length>1)elements=Array.from(elements[0].children) as HTMLElement[];
    const stamp=Date.now(), imported:Block[]=[];
    const infer=(text:string):BlockType=>/homework|to do|task/i.test(text)?"homework":/due|deadline|coming up/i.test(text)?"deadline":/read|chapter|pages|scene/i.test(text)?"reading":/question|focus|consider|think/i.test(text)?"focus":/vocabulary|key term/i.test(text)?"vocabulary":/resource|link|website/i.test(text)?"resource":/learning target|objective|i can/i.test(text)?"targets":"note";
    for(const el of elements){
      const text=(el.textContent||"").trim();if(!text)continue;
      const id=stamp+imported.length,tag=el.tagName;
      if(tag==="H1"){imported.push({id,type:"hero",label:"IMPORTED POST",title:text,body:""});continue}
      if(tag==="BLOCKQUOTE"){imported.push({id,type:"quote",title:"Quote",body:el.innerHTML});continue}
      if(tag==="OL"){imported.push({id,type:"steps",title:"Steps",body:Array.from(el.querySelectorAll("li")).map(x=>x.textContent?.trim()).filter(Boolean).join("\n")});continue}
      if(tag==="UL"){imported.push({id,type:"checklist",title:"Checklist",body:Array.from(el.querySelectorAll("li")).map(x=>x.textContent?.trim()).filter(Boolean).join("\n")});continue}
      if(tag==="P"&&!imported.length){imported.push({id,type:"intro",title:"",body:el.innerHTML});continue}
      const heading=el.matches("h2,h3,h4")?el:el.querySelector<HTMLElement>("h1,h2,h3,h4,strong,b");
      const title=(heading?.textContent||"").trim()||blockMeta[infer(text)].label;
      const clone=el.cloneNode(true) as HTMLElement;if(heading&&!el.matches("h2,h3,h4"))clone.querySelector("h1,h2,h3,h4,strong,b")?.remove();
      const body=el.matches("h2,h3,h4")?"":clone.innerHTML.trim()||text;
      imported.push({id,type:infer(`${title} ${text}`),title,body});
    }
    if(!imported.length){setImportMessage("No readable content was found in that HTML.");return}
    if(!imported.some(b=>b.type==="hero"))imported.unshift({id:stamp-1,type:"hero",label:"IMPORTED POST",title:"IMPORTED BLACKBAUD CONTENT",body:"Review and rename this page"});
    setBlocks(imported);setSelected(imported[0].id);setPostTitle("Imported Blackbaud post");setImportMessage(`${imported.length} editable blocks created.`);setTimeout(()=>{setImportOpen(false);setImportMessage("");setImportSource("")},900);
  };
  ...
  return <main className={`app-shell ${isExtension?"extension-shell":""}`}>
    ...
    <section className="workspace">
      ...
      <section className="stage">
        <div className="stagebar"><div className="device-switch"><button className={device==="desktop"?"active":""} onClick={()=>setDevice("desktop")}>▰ Desktop</button><button className={device==="mobile"?"active":""} onClick={()=>setDevice("mobile")}>▯ Mobile</button></div><label className="surface-picker">Surface <select value={surfaceKey} onChange={e=>setSurfaceKey(e.target.value as SurfaceKey)}>{Object.entries(surfaces).map(([key,s])=><option key={key} value={key}>{s.name}</option>)}</select></label><div className="history-buttons"><button onClick={undo} title="Undo">↶ Undo</button><button onClick={redo} title="Redo">↷ Redo</button></div><span>{surface.width}px</span></div>
      ...
      <aside className="inspector">
        ...
        <div className="checks"><div className="check-head"><div><span className="eyebrow">COMPATIBILITY</span><h3>{surface.name}</h3></div><span className="score">4/4</span></div><p className="surface-note">{surface.note}</p><p><i>✓</i> All export styles are inline</p><p><i>✓</i> Blackbaud-safe structure</p><p><i>✓</i> Strong heading hierarchy</p><p><i>✓</i> Accessible color contrast</p>{blocks.some(b=>b.width==="half")&&<p className="compat-warning"><i>!</i> Preview half-width blocks in your target Blackbaud editor; responsive behavior can vary by surface.</p>}{surfaceKey==="announcement"&&blocks.some(b=>b.width==="half")&&<p className="compat-warning"><i>!</i> Full-width blocks are recommended for announcements.</p>}</div>
