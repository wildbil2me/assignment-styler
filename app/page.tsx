  ...
  const insertIntoBlackbaud = async () => {
    ...
    try{
      ...
      const results=await chromeApi.scripting.executeScript({target:{tabId:tab.id,allFrames:true},world:"MAIN",args:[html],func:(markup:string)=>{
        ...
        if(ck?.setData){ck.setData(markup);return {inserted:true,adapter:"ckeditor-api"}}
        if(page.jQuery){
          ...
          if(kendo?.value){kendo.value(markup);kendo.trigger?.("change");return {inserted:true,adapter:"kendo-api"}}
        }
