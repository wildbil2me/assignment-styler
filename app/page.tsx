  ...
  return <main className={`app-shell ${isExtension?"extension-shell":""}`}>
    ...
    <section className="workspace">
      ...
      <aside className="inspector">
        ...
        <div className="export">{isExtension&&<div className="insert-actions"><button className={`insert-button ${insertStatus}`} onClick={()=>insertIntoBlackbaud("replace")} disabled={insertStatus==="working"}>{insertStatus==="working"?"Finding editor…":insertStatus==="success"?"✓ Added to Blackbaud":insertStatus==="error"?"Editor not found":"Replace editor content"}</button><button className="cursor-button" onClick={()=>insertIntoBlackbaud("cursor")} disabled={insertStatus==="working"}>Paste at cursor</button></div>}<button className="copy" onClick={copy}>{copied?"✓ Copied to clipboard":"Copy for Blackbaud"}</button><details><summary>View generated HTML</summary><textarea readOnly value={html}/></details>{exportHistory.length>0&&<details><summary>Export history ({exportHistory.length})</summary><div className="export-history">{exportHistory.map((x,i)=><button key={`${x.date}-${i}`} onClick={()=>navigator.clipboard.writeText(x.html)}><strong>{x.title}</strong><small>{x.date} · Click to copy</small></button>)}</div></details>}<small>{isExtension?"Insertion never publishes or saves the Blackbaud page. Preview before publishing.":"Paste into Blackbaud’s HTML editor, then preview before publishing."}</small></div>
      </aside>
    </section>
  </main>;
}
